// Package cli builds the orv command tree on top of the openreview library.
package cli
...
func Root() *cobra.Command {
	app := &App{cfg: openreview.DefaultConfig()}

	root := &cobra.Command{
		Use:   "orv",
		Short: "Browse ML papers on OpenReview",
		Long: `orv reads OpenReview through the public API at api2.openreview.net.
No API key required. Returns records as table, JSON, JSONL, CSV, TSV, or URLs.

orv is an independent tool and is not affiliated with OpenReview.`,
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = openreview.NewClient(a.cfg)
	return nil
}
...
func mapFetchErr(err error) error {
	...
	if errors.Is(err, openreview.ErrNotFound) {
